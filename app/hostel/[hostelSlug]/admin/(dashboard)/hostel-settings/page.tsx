...
export default function HostelSettingsPage() {
    ...
    if (loading) return (
        <div className="flex h-64 items-center justify-center font-bold text-[#6b7280] animate-pulse">
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="bg-slate-950/20 backdrop-blur-md border border-[#1f2937] rounded-2xl min-h-[500px] flex flex-col p-6 shadow-inner relative">
                ...
                {viewLevel !== "blocks" && (
                    <button onClick={goBack} className="absolute top-6 left-6 z-10 flex items-center gap-1.5 bg-[#020617] border border-[#1f2937] hover:border-blue-500 text-slate-300 font-bold px-4 py-2 rounded-lg transition-all shadow-sm">
                        ...
                        <div className="flex flex-wrap justify-center items-end gap-16 sm:gap-24 w-full">
                            {blocks.map((b, i) => (
                                <div key={i} className="flex flex-col items-center group relative">
                                    <div className="flex items-center gap-2 mb-4 bg-[#020617] p-2 rounded-xl shadow-lg border border-[#1f2937] z-10">
                                    ...
                                    >
                                        ...
                                        {/* Building Body */}
                                        <div className="w-40 min-h-[240px] bg-[#0b1220] border-x border-t border-[#1f2937] flex flex-col-reverse p-3 shadow-2xl group-hover:border-indigo-400 group-hover:shadow-indigo-500/20 transition-all rounded-b-md border-b-[8px] border-b-[#1f2937]">
                                    ...
                                    <div className="mt-4 text-xs font-bold text-[#6b7280] text-center uppercase tracking-wider space-y-1">
                ...
                {viewLevel === "floors" && activeBlock && (
                    <div className="flex-1 flex flex-col items-center justify-start pt-4">
                        <div className="w-full flex justify-between items-center gap-3 mb-10 border-b-2 border-[#1f2937] pb-4 ml-0 sm:ml-20">
                            <h3 className="font-bold text-[#6b7280] uppercase tracking-widest">Building Stack</h3>
                            <div className="flex gap-3">
                                ...
                                <button onClick={deleteFloor} className="flex items-center gap-1.5 bg-[#0b1220] border-2 border-[#1f2937] text-rose-500 hover:bg-rose-500/10 px-4 py-2 rounded-lg font-bold text-sm shadow-sm"><Trash2 className="w-4 h-4"/> Remove Top</button>
                            </div>
                        </div>

                        {/* Stacked floor visual editor */}
                        <div className="flex flex-col-reverse justify-end items-center gap-1.5 bg-slate-200/50 p-8 rounded-t-xl border-x-[6px] border-t-[6px] border-[#1f2937] min-w-[340px] shadow-inner relative">
                            ...
                            {activeBlock.floors.map((f, i) => (
                                <div 
                                    ...
                                    className="w-[300px] sm:w-[400px] h-16 sm:h-20 bg-[#0b1220] border-4 border-[#1f2937] rounded-sm flex justify-between items-center px-6 cursor-pointer hover:bg-[#8b5cf6]/10 hover:border-indigo-500 group transition-all"
                                >
                                    <span className="text-2xl font-black text-[#f9fafb] group-hover:text-blue-600 transition-colors">Floor {f.floorNo}</span>
                                    <div className="flex gap-2">
                                        <span className={`font-bold text-xs px-2 py-1.5 rounded-lg border-2 ${f.occupiedBeds && f.occupiedBeds > 0 ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-slate-100 border-[#1f2937] text-slate-400'}`}>
                            ...
                            <div className="w-[320px] sm:w-[420px] h-3 bg-slate-600 mt-2 rounded"></div>
                        </div>
                    </div>
                )}
                ...
                {viewLevel === "rooms" && activeBlock && activeFloor && (
                    <div className="flex-1 flex flex-col items-center justify-start pt-4">
                        <div className="w-full flex justify-between items-center gap-3 mb-10 border-b-2 border-[#1f2937] pb-4 ml-0 sm:ml-20">
                            <h3 className="font-bold text-[#6b7280] uppercase tracking-widest">Floor Plan Editor</h3>
                            <div className="flex gap-3">
                                ...
                                <button onClick={deleteRoom} className="flex items-center gap-1.5 bg-[#0b1220] border-2 border-[#1f2937] text-rose-500 hover:bg-rose-500/10 px-4 py-2 rounded-lg font-bold text-sm shadow-sm"><Trash2 className="w-4 h-4"/> Remove Room</button>
                            </div>
                        </div>

                        {/* Top Down Floor Plan */}
                        <div className="bg-slate-200/80 p-8 sm:p-12 border-[8px] border-slate-400 rounded-lg max-w-4xl w-full flex flex-wrap gap-4 sm:gap-6 justify-center shadow-inner relative">
                            {activeFloor.rooms.length === 0 && <span className="font-bold text-[#6b7280] uppercase py-10 w-full text-center">Empty floor</span>}
                            
                            {activeFloor.rooms.map((r, i) => (
                                <div 
                                    key={i} 
                                    onClick={() => { setActiveRoomIndex(i); setViewLevel("beds"); }}
                                    className={`group cursor-pointer bg-[#0b1220] border-4 ${r.occupiedBeds && r.occupiedBeds > 0 ? "border-emerald-400" : "border-[#1f2937] hover:border-indigo-500"} w-32 h-32 sm:w-40 sm:h-40 flex flex-col items-center justify-center transition-all hover:scale-105 shadow-md relative`}
                                >
                                    ...
                                    <input 
                                        ...
                                        className="text-center font-black text-3xl bg-[#020617] border-none outline-none focus:ring-2 focus:ring-[#8b5cf6] rounded p-1 w-full text-[#f9fafb] transition-all mt-4 mb-2"
                                    />
                ...
                {viewLevel === "beds" && activeBlock && activeFloor && activeRoom && (
                    <div className="flex-1 flex flex-col items-center justify-start pt-4">
                        <div className="w-full flex justify-end gap-3 mb-10 border-b-2 border-[#1f2937] pb-4">
                            <h3 className="font-bold text-[#6b7280] uppercase tracking-widest w-full self-center ml-0 sm:ml-20">Room Bed Capacity</h3>
                        </div>

                        {/* Top down room plan for beds */}
                        <div className="bg-[#0b1220] border-[8px] border-slate-400 rounded-lg p-10 max-w-2xl w-full flex flex-col items-center shadow-inner relative min-h-[400px]">
                            {/* Room door marker */}
                            ...
                            <div className="mt-auto flex items-center gap-6 bg-[#020617] border-2 border-[#1f2937] p-4 rounded-2xl w-full justify-between shadow-sm">
                                ...
                                <div className="flex gap-4 pr-2">
                                    ...
                                    <div className="w-16 flex items-center justify-center font-black text-2xl text-[#f9fafb] bg-[#0b1220] border-2 border-[#1f2937] rounded-xl shadow-sm">{activeRoom.capacity}</div>
            ...
            <div className="bg-slate-900 border border-[#1f2937] rounded-2xl p-6 shadow-lg">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex flex-col">
                        ...
                        <span className="font-semibold text-xs text-[#6b7280] mt-1">Configure automated notifications for renewals</span>
                    </div>
                    <label className="flex items-center gap-2 cursor-pointer bg-slate-950 px-4 py-2 rounded-xl border border-[#1f2937] shadow-lg">
                        <span className={`text-sm font-black ${whatsappEnabled ? 'text-blue-400' : 'text-[#6b7280]'} uppercase tracking-widest`}>{whatsappEnabled ? 'ON' : 'OFF'}</span>
                ...
                {whatsappEnabled && (
                    <div className="mt-6 border-t-2 border-[#1f2937] pt-6">
                        <label className="block font-bold text-[#6b7280] uppercase tracking-widest text-xs mb-3">Message Format</label>
                        <textarea 
                            value={whatsappTemplate} 
                            onChange={e => setWhatsappTemplate(e.target.value)} 
                            rows={3} 
                            className="w-full bg-[#0b1220] border-2 border-[#1f2937] rounded-xl p-4 text-sm font-semibold focus:border-[#8b5cf6] outline-none resize-y"
                        />
                    </div>
                )}
            </div>
