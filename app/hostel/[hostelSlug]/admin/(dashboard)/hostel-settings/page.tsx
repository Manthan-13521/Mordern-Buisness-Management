        ...
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="bg-slate-950/20 backdrop-blur-md border border-white/5 rounded-2xl min-h-[500px] flex flex-col p-6 shadow-inner relative">
                ...
                {viewLevel === "floors" && activeBlock && (
                    <div className="flex-1 flex flex-col items-center justify-start pt-4">
                        <div className="w-full flex justify-between items-center gap-3 mb-10 border-b-2 border-slate-200 dark:border-slate-700 pb-4 ml-0 sm:ml-20">
                        ...
                        <div className="flex flex-col-reverse justify-end items-center gap-1.5 bg-slate-200/50 dark:bg-slate-800 p-8 rounded-t-xl border-x-[6px] border-t-[6px] border-slate-300 dark:border-slate-600 min-w-[340px] shadow-inner relative">
                            ...
                            {activeBlock.floors.map((f, i) => (
                                <div 
                                    ...
                                    className="w-[300px] sm:w-[400px] h-16 sm:h-20 bg-white dark:bg-slate-800 border-4 border-slate-400 dark:border-slate-500 rounded-sm flex justify-between items-center px-6 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-500/10 dark:hover:bg-slate-700 hover:border-indigo-500 group transition-all"
                                >
                                    <span className="text-2xl font-black text-slate-800 dark:text-white group-hover:text-blue-600 dark:text-blue-400 transition-colors">Floor {f.floorNo}</span>
                                    <div className="flex gap-2">
                                        ...
                                        <span className="font-bold text-slate-400 uppercase text-xs group-hover:text-blue-500 bg-slate-100 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-lg">{f.rooms.length} Rooms &gt;</span>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="bg-slate-200 dark:bg-slate-700/80 p-8 sm:p-12 border-[8px] border-slate-400 dark:border-slate-500 rounded-lg max-w-4xl w-full flex flex-wrap gap-4 sm:gap-6 justify-center shadow-inner relative">
                            ...
                            {activeFloor.rooms.map((r, i) => (
                                <div 
                                    ...
                                    className={`group cursor-pointer bg-white dark:bg-slate-800 border-4 ${r.occupiedBeds && r.occupiedBeds > 0 ? "border-emerald-400" : "border-slate-300 dark:border-slate-600 hover:border-indigo-500"} w-32 h-32 sm:w-40 sm:h-40 flex flex-col items-center justify-center transition-all hover:scale-105 shadow-md relative`}
                                >
                                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest absolute top-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded">Setup</span>
                                    
                                    <input 
                                        value={r.roomNo}
                                        onChange={e => updateRoomNo(activeBlockIndex!, activeFloorIndex!, i, e.target.value)}
                                        onClick={e => e.stopPropagation()}
                                        className="text-center font-black text-3xl bg-slate-100 dark:bg-slate-900 border-none outline-none focus:ring-2 focus:ring-indigo-500 rounded p-1 w-full text-slate-800 dark:text-slate-100 transition-all mt-4 mb-2"
                                    />
                ...
                {viewLevel === "beds" && activeBlock && activeFloor && activeRoom && (
                    <div className="flex-1 flex flex-col items-center justify-start pt-4">
                        <div className="w-full flex justify-end gap-3 mb-10 border-b-2 border-slate-200 dark:border-slate-700 pb-4">
                        ...
                        <div className="bg-white dark:bg-slate-800 border-[8px] border-slate-400 dark:border-slate-500 rounded-lg p-10 max-w-2xl w-full flex flex-col items-center shadow-inner relative min-h-[400px]">
                            {/* Room door marker */}
                            ...
                            <div className="mt-auto flex items-center gap-6 bg-slate-100 dark:bg-slate-900 border-2 border-slate-300 dark:border-slate-700 p-4 rounded-2xl w-full justify-between shadow-sm">
                                <span className="font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest text-sm pl-4">Add or Remove Beds</span>
                                <div className="flex gap-4 pr-2">
                                    <button onClick={deleteBed} className="w-12 h-12 bg-white dark:bg-slate-800 border-2 border-rose-200 text-rose-500 font-black text-2xl flex items-center justify-center rounded-xl hover:bg-rose-50 transition-colors shadow-sm">-</button>
                                    <div className="w-16 flex items-center justify-center font-black text-2xl text-slate-800 dark:text-white bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-600 rounded-xl shadow-sm">{activeRoom.capacity}</div>
                                    <button onClick={addBed} className="w-12 h-12 bg-white dark:bg-slate-800 border-2 border-emerald-200 text-emerald-500 font-black text-2xl flex items-center justify-center rounded-xl hover:bg-emerald-50 transition-colors shadow-sm">+</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {whatsappEnabled && (
                    <div className="mt-6 border-t-2 border-slate-200 dark:border-slate-700 pt-6">
                        ...
                        <textarea 
                            ...
                            rows={3} 
                            className="w-full bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-600 rounded-xl p-4 text-sm font-semibold focus:border-indigo-500 outline-none resize-y"
                        />
                    </div>
                )}
            </div>
