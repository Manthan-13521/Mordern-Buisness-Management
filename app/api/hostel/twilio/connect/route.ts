...
export async function POST(req: Request) {
    try {
        ...
        if (!sid || !authToken || !whatsappNumber || !testPhone) {
            return NextResponse.json({ error: "sid, authToken, whatsappNumber, and testPhone are required" }, { status: 400 });
        }
        ...
        try {
            ...
            return NextResponse.json({ error: "Invalid Twilio credentials format." }, { status: 400 });
        }
        ...
        return NextResponse.json({ success: true, message: "Twilio connected! Test message sent.", whatsappNumber: fromNumber });
    } catch (error: any) {
