...
export default function LogsPage() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/logs?type=all&memberType=${selectedType}&t=${Date.now()}`, { cache: 'no-store' })
            .then((res) => res.json())
            .then((data) => {
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <button
                        ...
                        className="inline-flex items-center rounded-xl bg-white/5 border border-white/10 px-4 py-2.5 text-sm font-bold text-slate-300 shadow-lg hover:bg-white/10 transition-all"
                    >
            ...
            <div className="max-w-md relative">
                ...
                <input
                    type="text"
                    className="block w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pl-10 text-white placeholder:text-slate-500 focus:ring-2 focus:ring-blue-500 sm:text-sm transition-all"
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                {/* Registrations Table */}
                <div className="overflow-hidden border border-white/5 rounded-2xl bg-slate-900 shadow-2xl flex flex-col max-h-[700px]">
                    <div className="p-4 border-b border-white/5 bg-white/5 flex items-center gap-2">
                        <UserPlus className="h-5 w-5 text-blue-500" />
                        <h2 className="text-lg font-bold text-white uppercase tracking-tight">Registrations</h2>
                    </div>
                    <div className="overflow-y-auto flex-1">
                        <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-800">
                            <thead className="bg-slate-950/50 backdrop-blur-md sticky top-0 z-10 shadow-sm">
                                <tr>
                                    <th className="py-3.5 pl-4 pr-3 text-left text-xs font-bold text-slate-400 uppercase tracking-wider sm:pl-6">Time</th>
                                    <th className="px-3 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Member</th>
                                    <th className="px-3 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Action</th>
                                </tr>
                            </thead>
                    ...
                    {/* Pagination Footer */}
                    {totalRegPages > 1 && (
                        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-800 bg-gray-50 bg-white dark:bg-white/5 dark:backdrop-blur-md dark:border dark:border-white/10 shadow-lg/50">
                            ...
                            <div className="flex space-x-2">
                                <button
                                    ...
                                    disabled={regPage === 1}
                                    className="inline-flex items-center rounded-md bg-white bg-white dark:bg-white/5 dark:backdrop-blur-md dark:border dark:border-white/10 shadow-lg px-3 py-1 text-sm font-semibold text-gray-900 dark:text-gray-300 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    Prev
                                </button>
                                <button
                ...
                <div className="overflow-hidden border border-white/5 rounded-2xl bg-slate-900 shadow-2xl flex flex-col max-h-[700px]">
                    <div className="p-4 border-b border-white/5 bg-white/5 flex items-center gap-2">
                        ...
                        <h2 className="text-lg font-bold text-white uppercase tracking-tight">Entry Scans</h2>
                    </div>
                    <div className="overflow-y-auto flex-1">
                        <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-800">
                            <thead className="bg-background sticky top-0 z-10 shadow-sm border-b border-white/10 text-white">
                                <tr>
                                ...
                                {loading ? (
                                    ...
                                    Object.entries(groupedEntries).map(([dateLabel, logsForDate]) => (
                                        <React.Fragment key={dateLabel}>
                                            {/* Date Separator Row */}
                                            <tr className="bg-gray-100 bg-white dark:bg-white/5 dark:backdrop-blur-md dark:border dark:border-white/10 shadow-lg/80 border-y border-gray-200 dark:border-gray-700">
