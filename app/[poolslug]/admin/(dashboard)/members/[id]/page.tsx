...
function ExpiryBanner({ member }: { member: Member }) {
    if (member.isDeleted) {
        return (
            <div className="rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center gap-3">
                <span className="text-2xl">🗑️</span>
                <div>
                    <p className="font-semibold text-gray-700 dark:text-gray-300">Member Deleted</p>
                    <p className="text-sm text-gray-500">Reason: {member.deleteReason ?? "manual"} · {member.deletedAt ? new Date(member.deletedAt).toLocaleDateString("en-IN") : ""}</p>
                </div>
            </div>
        );
    }
    ...
    if (isActuallyExpired) {
        return (
            <div className="rounded-lg bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 px-4 py-3 flex items-center gap-3">
                <span className="text-2xl">⚠️</span>
                <div>
                    <p className="font-semibold text-red-700 dark:text-red-400">Plan Expired</p>
                    <p className="text-sm text-red-600 dark:text-red-500">Expired on {endDate.toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}</p>
                </div>
            </div>
        );
    }
    ...
    if (daysLeft <= 7) {
        return (
            <div className="rounded-lg bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 px-4 py-3 flex items-center gap-3">
                <span className="text-2xl">⏰</span>
                <div>
                    <p className="font-semibold text-amber-700 dark:text-amber-400">Expiring Soon — {daysLeft} day{daysLeft !== 1 ? "s" : ""} left</p>
                    <p className="text-sm text-amber-600 dark:text-amber-500">Valid till {endDate.toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="rounded-lg bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800 px-4 py-3 flex items-center gap-3">
            <span className="text-2xl">✅</span>
            <div>
                <p className="font-semibold text-green-700 dark:text-green-400">Active — {daysLeft} days remaining</p>
                <p className="text-sm text-green-600 dark:text-green-500">Valid till {endDate.toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}</p>
            </div>
        </div>
    );
}
                        ...
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                                <a href={`/api/members/${memberId}/pdf`} download
                                    className="inline-flex items-center gap-1.5 rounded-md bg-white dark:bg-gray-800 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 ring-1 ring-gray-300 dark:ring-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <Download className="h-3.5 w-3.5" /> ID Card
                                </a>
                                {member.planId?.hasTokenPrint && (
                                    <button onClick={handleReprint}
                                        className="inline-flex items-center gap-1.5 rounded-md bg-white dark:bg-gray-800 px-3 py-1.5 text-sm font-medium text-green-700 dark:text-green-400 ring-1 ring-green-300 dark:ring-green-800 hover:bg-green-50 dark:hover:bg-green-900/30">
                                ...
                                {member.isDeleted ? (
                                    ...
                                    <button onClick={handleDelete}
                                        className="inline-flex items-center gap-1.5 rounded-md bg-white dark:bg-gray-800 px-3 py-1.5 text-sm font-medium text-red-600 dark:text-red-400 ring-1 ring-red-300 dark:ring-red-800 hover:bg-red-50 dark:hover:bg-red-900/30">
            ...
            <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                    <h3 className="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                        <Package className="h-4 w-4 text-indigo-500" /> Equipment
                        {unreturned.length > 0 && (
                ...
                <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 focus:ring-2 focus:ring-indigo-500"
                        />
                        <button
                            ...
                            {unreturned.map(item => (
                                <div key={item._id} className="flex items-center justify-between rounded-lg bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/40 px-3 py-2">
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-1 rounded-md bg-white dark:bg-gray-800 px-2.5 py-1.5 text-xs font-medium text-green-700 dark:text-green-400 ring-1 ring-green-300 dark:ring-green-800 hover:bg-green-50 dark:hover:bg-green-900/30"
                                    >
                ...
                {returned.length > 0 && (
                    <div className="px-6 py-3">
                        ...
                        <div className="space-y-1.5">
                            {returned.map(item => (
                                <div key={item._id} className="flex items-center justify-between rounded-lg bg-gray-50 dark:bg-gray-800/50 px-3 py-2 opacity-60">
