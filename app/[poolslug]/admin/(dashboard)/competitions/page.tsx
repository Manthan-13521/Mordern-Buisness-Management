    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-2xl bg-white dark:bg-gray-900 shadow-2xl p-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Create Competition</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        <button type="button" onClick={onClose} className="rounded-lg px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 ring-1 ring-gray-300 dark:ring-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">Cancel</button>
                        <button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500 disabled:opacity-60">
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-start justify-between">
                <div>
                    <h3 className="text-base font-semibold text-gray-900 dark:text-white">{comp.name}</h3>
                ...
                <div>
                    ...
                    <div className="space-y-1.5">
                        {comp.winners.sort((a, b) => a.position - b.position).map(w => (
                            <div key={w.position} className="flex items-center gap-2 rounded-lg bg-gray-50 dark:bg-gray-800/50 px-3 py-2">
                                <span className={`text-lg ${medalColors[w.position - 1] ?? "text-gray-400"}`}>
                                    {w.position === 1 ? "🥇" : w.position === 2 ? "🥈" : w.position === 3 ? "🥉" : `#${w.position}`}
                                </span>
                                <p className="flex-1 text-sm font-medium text-gray-900 dark:text-white">{w.name}</p>
                                {w.timing && <p className="text-xs text-gray-500">{w.timing}s</p>}
                                {w.prize && <p className="text-xs text-indigo-600 dark:text-indigo-400 font-medium">{w.prize}</p>}
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        <div key={i} className="space-y-1.5">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                <input type="text" placeholder="Winner name" value={w.name}
                                    ...
                                    className="col-span-1 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-2.5 py-1.5 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500" />
                                <input type="number" step="0.01" placeholder="Time (s)" value={w.timing}
                                    ...
                                    className="rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-2.5 py-1.5 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500" />
                ...
                {comp.participants.length === 0 ? (
                    ...
                    <div className="space-y-1.5">
                        {comp.participants.map(p => (
                            <div key={p._id} className="flex items-center gap-2 rounded-lg bg-gray-50 dark:bg-gray-800/50 px-3 py-2">
                                ...
                                {p.position && <span className="text-xs font-bold text-indigo-600">#{p.position}</span>}
                            </div>
                        ))}
                    </div>
                )}
            ...
            {!comp.isCompleted && (
                <div className="flex gap-2 pt-2">
                    <input type="text" placeholder="Add participant name…" value={newParticipant} onChange={e => setNewParticipant(e.target.value)}
                        ...
                        className="flex-1 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500" />
        ...
        <div className="space-y-6">
            ...
            <div className="sm:flex sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{total} event{total !== 1 ? "s" : ""}</p>
                </div>
            ...
            <div className="flex gap-1 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg w-fit">
                {(["all", "upcoming", "completed"] as const).map(f => (
                    <button key={f} onClick={() => setFilter(f)}
                        className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors capitalize ${filter === f ? "bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm" : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"}`}>
                        {f}
                    </button>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="lg:col-span-2 space-y-3">
                    {loading ? (
                        <div className="text-center py-10"><RefreshCw className="animate-spin h-5 w-5 mx-auto text-indigo-500" /></div>
                        ...
                        <div key={c._id} onClick={() => setSelected(c)}
                            className={`rounded-xl border bg-white dark:bg-gray-900 p-4 cursor-pointer hover:shadow-md transition-all ${selected?._id === c._id ? "border-indigo-500 ring-1 ring-indigo-500" : "border-gray-200 dark:border-gray-800"}`}>
                            <div className="flex items-start justify-between">
                    ...
                    {totalPages > 1 && (
                        <div className="flex justify-between items-center pt-2">
                            <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1}
                                className="text-sm text-indigo-600 hover:underline disabled:opacity-40">← Prev</button>
                            <span className="text-xs text-gray-400">Page {page}/{totalPages}</span>
                            <button onClick={() => setPage(p => Math.min(totalPages, p+1))} disabled={page>=totalPages}
                                className="text-sm text-indigo-600 hover:underline disabled:opacity-40">Next →</button>
                        </div>
                    )}
                </div>

                {/* Detail Panel */}
                <div className="lg:col-span-3 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm p-5">
                    {selected ? (
